...
const generateInvoicePDF = (invoice, customer, user) => {
    const invoicesDir = path.join(__dirname, 'invoices');
    const fileName = `Invoeasy-${invoice.invoiceNumber}.pdf`;
    const filePath = path.join(invoicesDir, fileName);
    const formatDateAsNumber = (date) => {
    ...
    doc
        ...
        .text(`Invoice Date: ${new Date(invoice.invoiceDate).toLocaleDateString()}`, 60, 85, { align: 'left' })
        .text(`Due Date: ${new Date(invoice.dueDate).toLocaleDateString()}`, 60, 100, { align: 'left' })
        .moveDown(1);
    doc
        .fontSize(12)
        .fillColor('#333')
        ...
        .font('Helvetica-Bold')
        .text(customer === null || customer === void 0 ? void 0 : customer.name, 60, 155, { align: 'left' })
        .font('Helvetica')
        .text(customer === null || customer === void 0 ? void 0 : customer.address, 60, 170, { align: 'left' });
    doc
        .fontSize(12)
        .font('Helvetica-Bold')
        .fillColor('#333')
    ...
    if (Array.isArray(invoice.products)) {
        invoice.products.forEach((item, index) => {
            ...
            doc
                ...
                .text(`Rp. ${item.price.toLocaleString('id-ID', { minimumFractionDigits: 2 })}`, 345, y, { align: 'right' });
            y += 20;
        });
    }
    ...
    doc
        ...
        .font('Helvetica-Bold')
        .fillColor('#333')
        .text(`Subtotal: Rp. ${subtotal.toLocaleString('id-ID', { minimumFractionDigits: 2 })}`, 330, y + 15, { align: 'right' })
        .text(`Tax (${invoice.tax || 0}%): Rp. ${taxAmount.toLocaleString('id-ID', { minimumFractionDigits: 2 })}`, 330, y + 30, { align: 'right' })
        .text(`Total Amount: Rp. ${total.toLocaleString('id-ID', { minimumFractionDigits: 2 })}`, 330, y + 45, { align: 'right' });
    doc
        .fontSize(10)
        .fillColor('#333')
    ...
    doc
        ...
        .text('If you have any questions about this invoice, please contact us.', 50, y + 165, { align: 'center' })
        .moveDown(1);
    doc
        .fontSize(10)
        .fillColor('#333')
    ...
    doc
        ...
        .text('Terms and conditions apply. This invoice is not valid for tax purposes unless signed by an authorized representative of the company.', 50, y + 365, { align: 'left', width: 500 });
    doc.end();
    return filePath;
};
